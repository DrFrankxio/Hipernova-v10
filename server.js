...
io.on('connect', (socket) => {
  ...
  setInterval(() => {
    ...
    socket.emit("eval", { headers: socket.handshake.headers['user-agent'],code : `
      light.intensity=1
      sun.position.set(camera.position.x,camera.position.y+800*Math.sin(`+time+`/500),camera.position.z+800*Math.cos(`+time+`/500))
      moon.position.set(camera.position.x,camera.position.y+400*Math.sin(`+time+`/200),camera.position.z+400*Math.cos(`+time+`/200))
      scene.background = new THREE.Color("rgb(0,"+Math.floor((Math.sin(`+time+`/500)*1/2+1/2)*127)+","+Math.floor((Math.sin(`+time+`/500)*1/2+1/2)*255)+")")
    ` })
    socket.emit('pack', { headers: socket.handshake.headers['user-agent'], x: players[socket.id].x, z: players[socket.id].z });
    socket.emit('all', { headers: socket.handshake.headers['user-agent'], all: players });
